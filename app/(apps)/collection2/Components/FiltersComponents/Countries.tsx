...
const Countries = ({ setFilters, filters }: CountriesProps) => {
    ...
    const handleAllChange = (value: boolean | string) => {
        if (typeof value === "boolean") {
            ...
            setFilters({ ...filters, countries: countries.reduce((acc, country) => ({ ...acc, [country.code]: value }), {}) })
        }
    }
            ...
            {countries.map((country, index) => {
                return (
                    <div key={index} className='flex items-center gap-2'>
                        <Checkbox checked={filters.countries[country.code]} onCheckedChange={(value) => handleChange(value, country.code)} />
                        <p>{country.name}</p>
                    </div>
                )
            })}
